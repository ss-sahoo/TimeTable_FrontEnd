...
export default function Users() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const endpoint = instituteId ? `/auth/users/?institute_id=${instituteId}` : '/auth/users/';
          ...
          <div className="mb-6 border-b" style={{ borderColor: '#e5e7eb' }}>
            <div className="flex gap-6">
              ...
              <button
                ...
                className={`pb-3 px-1 text-sm font-medium transition-colors relative ${roleFilter === 'institute_admin' ? 'text-blue-600' : 'text-gray-500 hover:text-gray-700'
                  }`}
              >
                Admins
                <span className="ml-2 px-2 py-0.5 text-xs rounded-full" style={{
