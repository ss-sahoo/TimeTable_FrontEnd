...
export default function AdminBatchesContent() {
    ...
    const fetchBatches = async () => {
        ...
        try {
            setLoading(true);
            setError(null);
            const response = await api.get(`/timetable/batches/?center_id=${centerId}`);
            const allBatches = response.data.batches || [];
            // Filter batches by admin's center
            const filteredBatches = allBatches.filter((b: Batch) => String(b.center_id) === String(centerId));
    ...
    const fetchPrograms = async () => {
        if (!centerId) return;
        try {
            const response = await api.get(`/timetable/programs/?center_id=${centerId}`);
            const allPrograms = response.data.programs || [];
            // Filter programs by admin's center
            const filteredPrograms = allPrograms.filter((p: Program) => String(p.center_id) === String(centerId));
    ...
    const handleCreateProgram = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await fetchPrograms();
            setProgramData({ name: '', description: '', category: '' });
                ...
                <div className="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                    <div className="flex items-center justify-center min-h-screen p-4 text-center sm:p-0">
                        ...
                        <div className="relative transform overflow-hidden rounded-3xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                            <div className="bg-gradient-to-br from-indigo-600 to-violet-700 px-6 py-8 text-white">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h3 className="text-xl font-black tracking-tight">Create New Program</h3>
                                            <p className="text-indigo-100 text-xs font-bold opacity-90 uppercase tracking-widest">Define academic track</p>
                                        </div>
                                    </div>
                            ...
                            <form onSubmit={handleCreateProgram} className="p-8 space-y-5">
                                {programError && (
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        rows={3}
                                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold text-slate-700 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all resize-none"
                                ...
                                <div className="flex gap-3 pt-3">
                                    ...
                                    <button
                                        ...
                                        className="flex-[2] py-3.5 bg-indigo-600 text-white rounded-2xl text-sm font-black shadow-lg shadow-indigo-200 hover:shadow-indigo-300 transition-all flex items-center justify-center disabled:opacity-50 uppercase tracking-widest"
                                    >
                                        {creatingProgram ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Create Program'}
                                    </button>
                                </div>
                            </form>
                        </div>
