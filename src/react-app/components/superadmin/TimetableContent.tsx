...
import { CalendarDays, Plus, Edit, Trash2, Search, Filter, Clock, Building, MoreVertical, AlertCircle, GraduationCap } from "lucide-react";
...
const TimetableContent = () => {
  ...
  useEffect(() => {
    const fetchTimetables = async () => {
      ...
      setLoading(true);
      try {
        const response = await api.get(`/timetable/timetables/?institute_id=${instituteId}`);
  ...
  const filteredTimetables = timetables.filter(tt =>
    tt.batch_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    tt.subject?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    tt.institute_name?.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Master Schedule</h2>
          <p className="text-slate-500 dark:text-gray-400 text-sm">View and manage class schedules for {user?.institute?.name || user?.institute_name || 'your institute'}</p>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl shadow-sm border border-slate-200 dark:border-gray-700 overflow-hidden">
        <div className="p-6 border-b border-slate-100 dark:border-gray-700 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              type="text"
              placeholder="Search by batch, subject or institute..."
              value={searchQuery}
        ...
        {loading ? (
          <div className="flex flex-col justify-center items-center h-96 gap-4">
            ...
            <p className="text-slate-500 dark:text-gray-400 font-medium animate-pulse">Fetching schedules...</p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 dark:bg-gray-900/50">
                  <th className="p-6 text-xs font-bold text-slate-500 dark:text-gray-400 uppercase tracking-wider">Class & Subject</th>
                  <th className="p-6 text-xs font-bold text-slate-500 dark:text-gray-400 uppercase tracking-wider">Time Slot</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-gray-700">
                {filteredTimetables.map((tt) => (
                  <tr key={tt.id} className="hover:bg-slate-50/50 dark:hover:bg-gray-900/50 transition-colors group">
                    <td className="p-6">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 flex items-center justify-center shrink-0">
                          <CalendarDays size={22} />
                        </div>
                        <div className="min-w-0">
                          <p className="font-bold text-slate-900 dark:text-white truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{tt.subject || "General Class"}</p>
                          <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-gray-400 mt-1">
                            <GraduationCap size={12} />
                            <span>{tt.batch_name}</span>
                          </div>
                        </div>
                      </div>
                    </td>
                    <td className="p-6">
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-slate-500 dark:text-gray-400 font-bold uppercase tracking-wider">{tt.day_of_week}</p>
                      </div>
                    </td>
                    <td className="p-6">
                      <div className="flex items-center gap-2 text-sm font-medium text-slate-600 dark:text-gray-400">
                        <Building size={14} className="text-slate-400" />
                        {tt.institute_name || "N/A"}
                      </div>
                    </td>
          ...
          <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
            ...
            <p className="text-slate-500 dark:text-gray-400 mt-2 max-w-xs mx-auto">
              {searchQuery ? `We couldn't find any schedules matching "${searchQuery}"` : "Get started by creating your first class schedule."}
            </p>
          </div>
        )}
      </div>
