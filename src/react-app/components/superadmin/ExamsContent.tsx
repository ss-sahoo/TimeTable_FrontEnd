...
import { FileText, Plus, Edit, Trash2, Search, Filter, Calendar, Clock, Building, MoreVertical, AlertCircle } from "lucide-react";
...
const ExamsContent = () => {
  ...
  useEffect(() => {
    const fetchExams = async () => {
      ...
      setLoading(true);
      try {
        const response = await api.get(`/exams/exams/?institute_id=${instituteId}`);
  ...
  const filteredExams = exams.filter(exam =>
    ...
    exam.institute_name?.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Global Exam Registry</h2>
          <p className="text-slate-500 dark:text-gray-400 text-sm">Monitor and manage all examinations for {user?.institute?.name || user?.institute_name || 'your institute'}</p>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl shadow-sm border border-slate-200 dark:border-gray-700 overflow-hidden">
        ...
        {loading ? (
          <div className="flex flex-col justify-center items-center h-96 gap-4">
            ...
            <p className="text-slate-500 dark:text-gray-400 font-medium animate-pulse">Fetching exams...</p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 dark:bg-gray-900/50">
                  <th className="p-6 text-xs font-bold text-slate-500 dark:text-gray-400 uppercase tracking-wider">Exam Details</th>
                  <th className="p-6 text-xs font-bold text-slate-500 dark:text-gray-400 uppercase tracking-wider">Schedule</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-gray-700">
                {filteredExams.map((exam) => (
                  <tr key={exam.id} className="hover:bg-slate-50/50 dark:hover:bg-gray-900/50 transition-colors group">
                    <td className="p-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0">
                          <p className="font-bold text-slate-900 dark:text-white truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{exam.title}</p>
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 dark:bg-gray-700 text-slate-600 dark:text-gray-400 mt-1 uppercase tracking-wider">
                            {exam.status}
                          </span>
                        </div>
                      </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
            ...
            <p className="text-slate-500 dark:text-gray-400 mt-2 max-w-xs mx-auto">
              {searchQuery ? `We couldn't find any exams matching "${searchQuery}"` : "Get started by creating your first examination."}
            </p>
          </div>
        )}
      </div>
